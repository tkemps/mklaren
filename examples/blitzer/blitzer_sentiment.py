...
def process(dataset, outdir):
    ...
    y = vstack([y1, y2])
    inxs_tr = list(range(0, ndata))
    inxs_te = list(range(ndata, ndata + ndata_test))
    ...
    Vs = list([X[:, i].reshape((n, 1)) for i in range(X.shape[1])])
    if center:
        Vs = list(map(center_kernel_low_rank, Vs))
